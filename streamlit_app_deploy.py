"""
streamlit_app_deploy.py — Self-contained dashboard for Streamlit Community Cloud.

Differences from streamlit_app.py (local dev version):
  - No FastAPI dependency — reads JSON artifacts and model directly from disk
  - Live Scoring calls model.predict_proba() inline instead of POST /predict
  - Paths resolve relative to this file so they work on Streamlit Cloud
...
# ── Paths ──────────────────────────────────────────────────────────────────────
# Path(__file__) resolves correctly both locally and on Streamlit Cloud
...
.risk-low    { color: #10b981; background: rgba(16,185,129,0.13); border: 1px solid #10b981; padding: 2px 10px; border-radius: 20px; font-family: 'Space Mono', monospace; font-size: 0.8rem; }
.risk-medium { color: #f59e0b; background: rgba(245,158,11,0.13); border: 1px solid #f59e0b; padding: 2px 10px; border-radius: 20px; font-family: 'Space Mono', monospace; font-size: 0.8rem; }
.risk-high   { color: #ef4444; background: rgba(239,68,68,0.13);  border: 1px solid #ef4444; padding: 2px 10px; border-radius: 20px; font-family: 'Space Mono', monospace; font-size: 0.8rem; }
...
.stButton > button {
    background: linear-gradient(135deg, rgba(0,212,255,0.13), rgba(123,94,167,0.13));
    border: 1px solid rgba(0,212,255,0.38); color: #00d4ff;
...
def model_label(name): return MODEL_META.get(name, {}).get("label", name.replace("_", " ").title())
...
# ── Data loaders (cached so files are read once per session) ───────────────────
@st.cache_data
...
def load_feature_names() -> list:
    data = load_json("feature_names.json")
    # feature_names.json is a list, not a dict
...
with st.sidebar:
    ...
    model = load_model()
    color_status = "#10b981" if model else "#ef4444"
    label_status = "● Model Loaded" if model else "● Model Not Found"
    st.markdown(f'<span style="color:{color_status}; font-family:Space Mono; font-size:0.8rem;">{label_status}</span>', unsafe_allow_html=True)
    ...
    STACK<br>scikit-learn · Streamlit<br>Plotly · MLflow
    </div>
    """, unsafe_allow_html=True)
...
st.markdown('<div class="page-sub">LR · Random Forest · LightGBM · Real-time scoring · MLflow tracked</div>', unsafe_allow_html=True)
...
if page == "📊 Model Performance":
    ...
    with right:
        ...
        pr = metrics["pr_curve"]
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=pr["recall"], y=pr["precision"], mode="lines",
                                  line=dict(color=color, width=2.5),
                                  fill="tozeroy", fillcolor=hex_to_rgba(color),
    ...
    fig3 = go.Figure(go.Heatmap(
        ...
        colorscale=[[0, "#13151c"], [1, hex_to_rgba(color, 0.25)]], showscale=False, xgap=3, ygap=3,
    ))
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown(f"All three models trained on the same stratified split. Best model by **PR-AUC**: **{model_label(best)}**")
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    # Overlaid curves — load per-model metrics
    ...
    for i, name in enumerate(model_names):
        ...
        fig_cm = go.Figure(go.Heatmap(
            ...
            colorscale=[[0, "#13151c"], [1, hex_to_rgba(c, 0.25)]], showscale=False, xgap=3, ygap=3,
        ))
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown("Lower the threshold to catch more fraud (higher recall). Raise it to reduce false alarms (higher precision).")
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.info(f"At threshold **{threshold:.2f}**: ~{t_precision*100:.0f}% of flagged transactions are genuine laundering. You catch ~{t_recall*100:.0f}% of all laundering.")
...
# ══════════════════════════════════════════════════════════════════
    ...
    fi_raw  = load_json("feature_importance.json")
    metrics = load_json("metrics.json")
    # feature_importance.json is a list
    features = fi_raw if isinstance(fi_raw, list) else fi_raw.get("features", [])

    if not features:
        st.error("feature_importance.json not found in models/")
        st.stop()

    name  = metrics.get("model_name", "best model") if metrics else "best model"
    color = model_color(name)
    st.markdown(f"### Feature Importance — `{model_label(name)}`")
    st.markdown("Split-based importance: how often each feature is used across all trees.")

    df_fi = pd.DataFrame(features[:20]).sort_values("importance", ascending=True)
    fig = go.Figure(go.Bar(
        ...
        textposition="outside", textfont=dict(family="Space Mono", size=10, color="#6b7280"),
    ))
    fi_layout = {**LAYOUT, "height": max(350, len(df_fi) * 28), "xaxis_title": "Importance",
                 "yaxis": dict(gridcolor="#1f2330", linecolor="#1f2330", tickfont=dict(family="DM Sans", size=11))}
...
# ══════════════════════════════════════════════════════════════════
    ...
    if not model or not feature_names:
        st.error("Model or feature_names.json not found in models/")
        st.stop()
    ...
    with st.form("score_form"):
        ...
        with c2:
            ...
            transaction_hour  = st.slider("Transaction hour",     0, 23,  d["transaction_hour"])
        ...
        with c3:
            ...
            from_total = st.number_input("Account total volume",  value=float(d["from_account_total_volume"]))
    ...
    if submitted:
        # Compute cyclical encodings from the raw hour/day inputs
        hour_sin = math.sin(2 * math.pi * transaction_hour / 24)
        hour_cos = math.cos(2 * math.pi * transaction_hour / 24)
